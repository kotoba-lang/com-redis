(ns redis.main
  "Kotodama WASM entrypoint for the Redis clean-room actor (L4) — Clojure port.
  ...
  facts (`redis.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "RespMessage"      :plural "respmessages"      :id-prefix "redis_res"
    :fields [:firstByte :respType :payload :elementCount :length] :required [:firstByte :respType]
    :coerce {:elementCount :int :length :int}  :refs {}}
   {:entity "RedisCommand"     :plural "rediscommands"     :id-prefix "redis_red"
    :fields [:commandName :arity :flags :keyPosition] :required [:commandName :arity]
    :coerce {:arity :int :keyPosition :int}    :refs {}}
   {:entity "RedisKey"         :plural "rediskeys"         :id-prefix "redis_red"
    :fields [:keyName :dataType :ttl :encoding] :required [:keyName :dataType]
    :coerce {:ttl :int}                        :refs {}}
   {:entity "RedisDataType"    :plural "redisdatatypes"    :id-prefix "redis_red"
    :fields [:dataType :encoding :isBinary]    :required [:dataType :encoding]
    :coerce {:isBinary :bool}                  :refs {}}
   {:entity "RespProtocolVersion" :plural "respprotocolversions" :id-prefix "redis_res"
    :fields [:majorVersion :supportedSince]    :required [:majorVersion :supportedSince]
    :coerce {:majorVersion :int}               :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"redis.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "redis-compat" :tier tier :entities entities} 200])
